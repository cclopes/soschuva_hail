...
class GridMapDisplay(object):
    ...
    def plot_basemap(
        ...
        self.basemap.drawparallels(
            lat_lines, labels=[True, False, False, False], linewidth=0.25)
        self.basemap.drawmeridians(
            lon_lines, labels=[False, False, False, True], linewidth=0.25)
    ...
    def plot_latlon_level(
            ...
            plot_type='pcolormesh', vmin=None, vmax=None, norm=None, cmap=None,
            mask_outside=False, title=None, title_flag=True,
            axislabels=(None, None), axislabels_flag=True, colorbar_flag=True,
            colorbar_label=None, colorbar_orient='vertical', edges=True,
        ...
        plot_type : str
            Type of plot: pcolormesh, contour or quiver
        vmin, vmax : float
            Lower and upper range for the colormesh.  If either parameter is
            None, a value will be determined from the field attributes (if
            available) or the default values of -8, 64 will be used.
        ...
        fig : Figure
            Figure to add the colorbar to. None will use the current figure.
        rotate : bool
            Rotate data in 90 degrees. Good for longitudinal cutself.
        field_2 : str
            Second field to be plotted. Only applicable for quiver plots.
        dot_pos : tupple
            Position of the dot to be plotted
        zerodeg_height : float
            Height of 0 deg temperature
        minusfortydeg_height : float
            Height of -40 deg temperature
        zdh_col : str
            Color of 0 and -40 deg temperature lines
        """
        # parse parameters
        ax, fig = common.parse_ax_fig(ax, fig)
        vmin, vmax = common.parse_vmin_vmax(self.grid, field, vmin, vmax)
        ...
        if ang > 90.:
            delta_x = xy_res*np.cos((90.-ang)*np.pi/180.)  # was ang-90.
            delta_y = xy_res*np.sin((90.-ang)*np.pi/180.)  # was ang-90.
        else:
        ...
        if field_2 is not None:
            data2 = self.grid.fields[field_2]['data'][ind_z, ind_y, ind_x]
            data2 = np.reshape(data2, (nv_prof, nh_prof))
        ...
        if mask_outside:
            ...
            data2 = np.ma.masked_outside(data2, vmin, vmax)
        ...
        if plot_type == 'pcolormesh':
            pm = ax.pcolormesh(xyd, zd, data, vmin=vmin, vmax=vmax,
                               cmap=cmap, norm=norm, **kwargs)
            ax.plot(xy_1d, np.repeat(zerodeg_height, len(xy_1d)),
                    color=zdh_col, linestyle=':',
                    linewidth=2)
            ax.plot(xy_1d, np.repeat(minusfortydeg_height, len(xy_1d)),
                    color=zdh_col, linestyle=':',
                    linewidth=2)
            if zdh_col == 'k':
                fch_col = 'w'
                ...
                fch_col = 'k'
            ax.annotate(r'0$\degree$C', (xy_1d[1], zerodeg_height-0.35),
                        fontsize=12, fontstretch='condensed', weight='bold',
                        color=zdh_col,
                        bbox=dict(
                            boxstyle='round,pad=0.2', facecolor=fch_col,
                            alpha=0.35
                        ))
            ax.annotate(r'-40$\degree$C',
                        (xy_1d[1], minusfortydeg_height-0.35),
                        fontsize=12, fontstretch='condensed', weight='bold',
                        color=zdh_col,
                        bbox=dict(
                            boxstyle='round,pad=0.2', facecolor=fch_col,
        ...
        if plot_type == 'contour':
            if rotate:
                pm = ax.contour(zd[:-1, :-1] + z_res/2000.,
                                xyd[:-1, :-1] + xy_res/2000.,
                                data, linewidths=0.9,
                                colors='black', **kwargs)
            else:
                pm = ax.contour(xyd[:-1, :-1] + xy_res/2000.,
                                zd[:-1, :-1] + z_res/2000.,
                                data, levels=9, linewidths=0.9,
                                colors='black', **kwargs)
            ax.clabel(pm, inline=1, fontsize=9, fmt='%1.0f',
                      inline_spacing=2)

        if plot_type == 'quiver':
            # pm = ax.quiver(xyd[:-1:2, :-1:2] + z_res/2000.,
            #                zd[:-1:2, :-1:2] + xy_res/2000.,
            #                data[::2, ::2], data2[::2, ::2], angles='xy', color='k')
            # ax.quiverkey(pm, 0.935, 0.03, 10, '10 m/s',
            #              coordinates='figure')
            # pm = ax.barbs(xyd[:-1:2, :-1:2] + z_res/2000.,
            #               zd[:-1:2, :-1:2] + xy_res/2000.,
            #               data[::2, ::2], data2[::2, ::2], length=4,
            #               linewidths=0.4, flip_barb=True)
            pm = ax.streamplot(xyd[:-1, :-1] + z_res/2000.,
                               zd[:-1, :-1] + xy_res/2000.,
                               data, data2, color='k', density=1.5,
                               arrowsize=0.75, linewidth=(data2/10)**2 + 0.75)
            ax.plot((xy_1d[1], xy_1d[1] + lg_spc), (19.5, 19.5),
                    lw=(5/10)**2 + 0.75, color='k', zorder=20)
            ax.plot((xy_1d[1], xy_1d[1] + lg_spc), (18.3, 18.3),
                    lw=(10/10)**2 + 0.75, color='k', zorder=20)
            ax.plot((xy_1d[1], xy_1d[1] + lg_spc), (17.2, 17.2),
                    lw=(15/10)**2 + 0.75, color='k', zorder=20)
            ax.plot((xy_1d[1], xy_1d[1] + lg_spc), (16, 16),
                    lw=(20/10)**2 + 0.75, color='k', zorder=20)
            ax.plot((xy_1d[1], xy_1d[1] + lg_spc), (14.8, 14.8),
                    lw=(25/10)**2 + 0.75, color='k', zorder=20)
            ax.plot((xy_1d[1], xy_1d[1] + lg_spc), (13.4, 13.4),
                    lw=(30/10)**2 + 0.75, color='k', zorder=20)
            ax.annotate(
                ('       5\n       10\n       15\n       20\n       25\n       30'),
                (0, 13), horizontalalignment='left', fontsize=10,
                bbox=dict(boxstyle='round,pad=0.2', facecolor='w', alpha=0.5))
        ...
        for i in range(nh_prof):
            xticks_labels.append(
                '{:.2f}'.format(-lat_prof[i]) + '$\degree$S\n' +
                '{:.2f}'.format(-lon_prof[i]) + '$\degree$W')

        locs, labels = plt.xticks()
        nticks = len(locs)
        tick_freq = int(1.6*nh_prof/nticks)  # 1.6 for MultiDop, 2.2 for radar
        plt.xticks(
        ...
        if axislabels_flag:
            self._label_axes_latlon(axislabels, ax, rotate)

        if colorbar_flag:
            self.plot_colorbar(
                mappable=pm, label=colorbar_label, orientation=colorbar_orient,
                field=field, ax=ax, fig=fig, aspect=30)
        return

    def plot_colorbar(
            self, mappable=None, orientation='horizontal', label=None,
            cax=None, ax=None, fig=None, field=None, aspect=20):
        """
        Plot a colorbar.
    ...
    def _label_axes_latlon(self, axis_labels, ax, rotate=False):
