...
ggplot(data = selected_fams_df) +
  ...
  facet_wrap(~ case)
ggsave("trajectories_cases.png", width = 8.5, height = 4.25)
# ggsave("trajectories_cases_less.png", width = 7.5, height = 3.25) #-- For less plots
...
plt_size <- ggplot(data = selected_fams_df) +
  ...
  geom_vline(aes(xintercept = date_hailpad), linetype = "dashed") +
  labs(x = "Hour (UTC)", y = "Size (px)") +
  facet_grid(case ~ .) +
  theme(strip.text = element_blank())
#---------------------------------------------------------------------------------------------------------------------------------

#-- Saving variables
save.image("fortracc_data.RData")
