...
filenames_fams <- dir(path = "ForTraCC_Processing/families", pattern = "*.txt",
                      full.names = T)
...
filenames_cappis <- data.frame("path" = dir(path = "Data/RADAR/SR/level_2",
                               pattern = "cappi_CZ_03000_201*", full.names = T)) %>%
  mutate(date = str_extract(path, "201\\d\\d\\d\\d\\d_\\d\\d\\d\\d") %>%
  gsub("\\_", "", .))

dates_clusters_cappis <- intersect(filenames_cappis$date, filenames_clusters$date)
filenames_clusters <- filenames_clusters %>%
    filter(date %in% dates_clusters_cappis) %>%
    pull(path) %>%
    as.character
filenames_cappis <- filenames_cappis %>%
    filter(date %in% dates_clusters_cappis) %>%
    pull(path) %>%
    as.character
...
data_fams <- filenames_fams %>%
    purrr::map(~read_table(.x,
               col_names = c("n", "year", "month", "day", "hour", "min",
                             "lat", "lon", "size", "dsize", "pmed", "dpmed",
                             "pmax", "dpmax", "pmax9", "dpmax9", "frac",
                             "vel", "dir", "t_ini", "class", "sys", "sys_ant1",
                             "sys_ant2", "sys_ant3", "sys_ant4", "sys_ant5", "sys_ant6",
                             "sys_ant7", "sys_ant8", "sys_ant9", "sys_ant10"))) %>%
  purrr::map(~unite(.x, sys_ant1, sys_ant2, sys_ant3, sys_ant4, sys_ant5, sys_ant6, sys_ant7,
                    sys_ant8, sys_ant9, sys_ant10, col = "sys_ant", sep = ",")) %>%
...
#- If the code below doesn't work, you don't have enough memory (I think)...
...
#-- Not exactly on the same point! Considering a 0.025 uncertainty
selected_clusters <- which(dates_clusters_cappis %in% data_hailpads$date_arqs, arr.ind = T) %>%
                     data_clusters[.]
selected_lat <- purrr::map(as.list(data_hailpads$lat),
                           ~which(lat_vector < (.x+0.025) & lat_vector > (.x-0.025)))
selected_lon <- purrr::map(as.list(data_hailpads$lon),
                           ~which(lon_vector < (.x+0.025) & lon_vector > (.x-0.025)))
...
#-- This command may have problems if there's more than 1 family on the range defined above
selected_sys <- purrr::map(selected_sys, ~.x[which(!is.na(.x))]) %>%
    map_chr(unique)

#- Step 3: Find families correspondent to these systems
selected_fams <- data_fams %>%
    map2(., data_hailpads$date, ~filter(.x, date == .y)) %>%
    map2(., selected_sys, ~filter(.x, sys == .y)) %>%
    purrr::map(~select(.x, n)) %>%
    unlist %>% as.character()
...
problem_fams_start <- selected_fams %>%
    purrr::map(~group_by(.x, n)) %>%
    purrr::map(~slice(.x, 1)) %>%
    map_df(~filter(.x, class %in% c("S", "C"))) %>%
    ungroup() #%>% select(date, lat, lon, class)

problem_fams_end <- selected_fams %>%
    purrr::map(~group_by(.x, n)) %>%
    purrr::map(~slice(.x, n())) %>%
    map_df(~filter(.x, class %in% c("S", "M"))) %>%
    ungroup() #%>% select(date, lat, lon, class)
...
#-- 35dBZ
#--- Start
fam_nm1 <- data_fams[[3]] %>% filter(n == 46) %>%
    filter(date < problem_fams_start$date[1])
selected_fams[[3]] <- rbind(fam_nm1, selected_fams[[3]])

fam_nm1 <- data_fams[[5]] %>% filter(n == 69) %>%
    filter(date < (problem_fams_start$date[2]))
selected_fams[[5]] <- rbind(fam_nm1, selected_fams[[5]])

#--- End
fam_np1 <- data_fams[[1]] %>% filter(n == 138) %>%
    filter(date > (problem_fams_end$date[1]))
selected_fams[[1]] <- rbind(selected_fams[[1]], fam_np1)
fam_np1 <- data_fams[[1]] %>% filter(n == 169) %>%
    filter(date > (problem_fams_end$date[1] + 50*60))
selected_fams[[1]] <- rbind(selected_fams[[1]], fam_np1)
fam_np1 <- data_fams[[1]] %>% filter(n == 247) %>%
    filter(date > (problem_fams_end$date[1] + 120*60))
selected_fams[[1]] <- rbind(selected_fams[[1]], fam_np1)

fam_np1 <- data_fams[[4]] %>% filter(n == 279) %>%
    filter(date > (problem_fams_end$date[3]))
...
fam_np1 <- data_fams[[5]] %>% filter(n == 89) %>%
    filter(date > (problem_fams_end$date[4]))
