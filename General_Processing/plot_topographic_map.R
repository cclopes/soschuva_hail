...
coords <- data.frame(lon = seq(-50, -40, 0.1), 
                     lat = seq(-28, -18, 0.1))
# Projection
...
ggplot() +
  ...
  geom_sf(data = cities, fill = NA, size = 0.25) +
  geom_sf(data = cities_highlight, fill = NA, size = 0.5, colour = "gray20") +
  geom_point(data = radars_circles %>% filter(radar == "XPOL"), 
             aes(lon, lat, color = radar), 
             shape = 17, size = 2) +
  geom_path(data = radars_circles  %>% filter(radar == "XPOL"),
            aes(circle_80_lon, circle_80_lat, color = radar, size = "80")) +
  scale_color_manual(name = "Radar", 
                     values = c("#352C51", "#352C51", "#352C51"),
                     guide = guide_legend(order = 3)) +
  new_scale_color() +
  geom_point(data = hailpads, aes(lon, lat, color = collected), 
             shape = 4, size = 2, stroke = 1, inherit.aes = F) +
  scale_color_manual(name = "Hailpads",
