library(readr); library(ggplot2); library(ggalt); library(tidyverse); library(grid); library(cowplot); library(reshape2)
...
plt_b <- ggplot(hids %>% select(HID, ZDR_low, ZDR_high), aes(x = ZDR_low, xend = ZDR_high, y = HID, group = HID)) +
  ...
  scale_y_discrete(limits = rev(hids$HID), name = "") + scale_x_continuous(name = "ZDR (dBZ)")
  
plt_c <- ggplot(hids %>% select(HID, KDP_low, KDP_high), aes(x = KDP_low, xend = KDP_high, y = HID, group = HID)) +
  geom_dumbbell(color = "darkgreen", size = 1) +
  scale_y_discrete(limits = rev(hids$HID), name = "") + scale_x_continuous(name = "KDP (deg/km)")
...
plt <- ggplot(tak_plot, aes(x = Var1, y = Var2)) +
  ...
  geom_contour(aes(z = value), colour = "black", breaks = c(-65,-55,-45,-35,-25,-15,-5,5,15,25,35,45,55,65)) +
...
plt <- ggplot(files) +
  geom_tile(aes(y = hour, x = date, fill = Radar), position = position_dodge(width = 0.5)) +
  scale_fill_manual(values = c("slateblue", "peru")) +
  labs(title = "'level_0' Weather Radar Data Availability", x = "Case", y = "Hour UTC") +
...
sr_elevs <- c(0.49987793, 0.99975586, 1.99951172, 2.99926758, 3.99902344, 4.9987793, 5.99853516, 6.99829102, 7.99804688, 8.99780273,
              9.99755859, 12.00256348, 14.0020752, 16.00158691, 18.00109863)
xpol_elevs <- c(.5, 1.8, 3.1, 4.4, 5.7, 7, 8.3, 9.6, 10.9, 13, 15, 18, 22, 26, 32, 40, 55)

cth_scan <- merge(r, cth_elevs) %>% rename(r = x, elev = y) %>% 
  mutate(h = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin(elev*pi/180)) - ke*a) %>% 
  mutate(h_up = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin((elev+0.5)*pi/180)) - ke*a) %>% 
  mutate(h_down = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin((elev-0.5)*pi/180)) - ke*a) %>% 
  mutate(elev = as.factor(elev))

sr_scan <- merge(r, sr_elevs) %>% rename(r = x, elev = y) %>% 
  mutate(h = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin(elev*pi/180)) - ke*a) %>% 
  mutate(h_up = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin((elev+1)*pi/180)) - ke*a) %>% 
  mutate(h_down = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin((elev-1)*pi/180)) - ke*a) %>% 
  mutate(elev = round(elev, 1) %>% as.factor(.))

xpol_scan <- merge(r, xpol_elevs) %>% rename(r = x, elev = y) %>% 
  mutate(h = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin(elev*pi/180)) - ke*a) %>% 
  mutate(h_up = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin((elev+0.65)*pi/180)) - ke*a) %>% 
  mutate(h_down = sqrt(r^2 + (ke*a)^2 + 2*r*ke*a*sin((elev-0.65)*pi/180)) - ke*a) %>% 
  mutate(elev = round(elev, 1) %>% as.factor(.))
...
ggplot(cth_scan, aes(x = r)) +
  ...
  labs(title = "FCTH Scan Strategy", x = "Range (km)", y = "Height (km)", 
       fill = "Elevation (deg)", color = "Elevation (deg)") +
...
ggplot(sr_scan, aes(x = r)) +
  geom_ribbon(aes(ymax = h_up, ymin = h_down, fill = elev, color = elev), alpha = 0.6, size = 0.1) +
  geom_line(aes(y = h, color = elev), size = 1, linetype = "dotted") +
  labs(title = "São Roque Scan Strategy", x = "Range (km)", y = "Height (km)", 
       fill = "Elevation (deg)", color = "Elevation (deg)") +
...
ggplot(xpol_scan, aes(x = r)) +
  geom_ribbon(aes(ymax = h_up, ymin = h_down, fill = elev, color = elev), alpha = 0.6, size = 0.1) +
  geom_line(aes(y = h, color = elev), size = 1, linetype = "dotted") +
  labs(title = "UNICAMP XPOL Scan Strategy", x = "Range (km)", y = "Height (km)", 
       fill = "Elevation (deg)", color = "Elevation (deg)") +
