...
selected_clusters <- purrr::map(purrr::map(selected_fams, ~select(.x, date) %>% unlist), 
                         ~which(ymd_hm(dates_clusters_cappis) %in% .x, arr.ind = T) %>% data_clusters[.]) %>% 
  map2(., purrr::map(selected_fams, ~filter(.x, date %in% ymd_hm(dates_clusters_cappis))) %>% purrr::map(., select, sys),
...
data_brasildat <- read_table("Lightning_Processing/filenames_brasildat_less", col_names = F) %>% unlist() %>%  #-- For less plots 
  purrr::map(read_csv) %>% purrr::map(~`colnames<-`(.x, c("date", "lat", "lon", "z", "peak_curr", "class", "axis_bigger", #-- Reading files
  ...
  map2(., purrr::map(selected_fams, ~filter(.x, date %in% ymd_hm(dates_clusters_cappis)) %>% select(date) %>% unlist), 
