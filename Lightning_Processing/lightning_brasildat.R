...
#-- Updating selected_clusters and creating selected_latlon with the whole families
selected_clusters <- map(map(selected_fams, ~select(.x, date) %>% unlist), 
                         ~which(ymd_hm(dates_clusters_cappis) %in% .x, arr.ind = T) %>% data_clusters[.]) %>% 
  map2(., map(selected_fams, ~filter(.x, date %in% ymd_hm(dates_clusters_cappis))) %>% map(., select, sys),
...
selected_latlon <- modify_depth(selected_clusters, 2, mutate, 
  ...
  map2(., map(selected_fams, ~filter(.x, date %in% ymd_hm(dates_clusters_cappis)) %>% 
                                               select(date) %>% unlist), ~set_names(.x, as_datetime(.y))) %>% 
  modify_depth(., 2, mutate, row_m1 = row - 0.01, row_p1 = row + 0.01, col_m1 = col - 0.01, col_p1 = col + 0.01,
...
# selected_latlon <- map2(selected_latlon, map(selected_fams, ~filter(.x, date %in% ymd_hm(dates_clusters_cappis)) %>% 
#                                                select(date) %>% unlist), 
...
data_brasildat <- read_table("Lightning_Processing/filenames_brasildat", col_names = F) %>% unlist() %>% 
  ...
  map2(., map(selected_fams, ~filter(.x, date %in% ymd_hm(dates_clusters_cappis)) %>% select(date) %>% unlist), 
       ~filter(.x, period %in% .y)) %>% #-- Selecting the same times as the radar
  map(., ~group_by(.x, period) %>% nest) %>% #-- Separating into a nested list (as selected_latlon)
  map(., ~set_names(.x$data, .x$period))
...
selected_latlon <- map2(selected_latlon, data_brasildat, ~.x[names(.y)]) #-- Selecting matching dates between clusters and
                                                                         #-- lightning data
data_brasildat <- map2(data_brasildat, selected_latlon, #-- Matching with lat, lon with 0.02 degree "uncertainty"
...
qte_total <- totais %>% group_by(case, class) %>% 
  count %>% ungroup() %>%
  mutate(class = paste("Total", class, "=", n)) %>% select(case, class)
